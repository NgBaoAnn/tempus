...
class QuoteRepository(context: Context) {
    ...
    fun getTodayQuote(): QuoteDto {
        ...
        // Kiểm tra xem đã có quote cho hôm nay chưa
        ...
        // Nếu đã cache quote cho hôm nay, trả về quote đó
        if (cachedDate == todayString && cachedText != null) {
            return QuoteDto(
                text = cachedText,
        ...
        // Nếu chưa có hoặc đã qua ngày, lấy quote mới
        val daySeed = today.year * 1000 + today.dayOfYear
        val newQuote = LocalQuotesProvider.getQuoteForSeed(daySeed)
